...
namespace SleepHunter.IO.Process
{
    internal sealed class ProcessMemoryScanner : IDisposable
    {
        ...
        public IntPtr Find(byte[] bytes, int size, long startingAddress = 0, long endingAddress = 0)
        {
            ...
            while (address <= end)
            {
                var queryResult = (int)NativeMethods.VirtualQueryEx(ProcessHandle, (IntPtr)address, out MemoryBasicInformation memoryInfo, (IntPtr)sizeofMemoryInfo);
                ...
                if (memoryInfo.Type == VirtualMemoryType.Private && memoryInfo.State == VirtualMemoryStatus.Commit && memoryInfo.Protect == VirtualMemoryProtection.ReadWrite)
                {
                    ...
                    {
                        var result = NativeMethods.ReadProcessMemory(ProcessHandle, memoryInfo.BaseAddress + (i * PageSize), searchBuffer, (IntPtr)searchBuffer.Length, out int numberOfBytesRead);
